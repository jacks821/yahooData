...
func TestSaveCrypto(t *testing.T) {
	...
	crypto := scripts.CryptoPair{Quote: quote,
		CirculatingSupply:   100000,
		VolumeLastDay:       100000000,
		VolumeAllCurrencies: 1000000000,
	}

	resp := scripts.saveCrypto(crypto)
	...
	if resp.CirculatingSupply != crypto.CirculatingSupply {
		t.Error("expected", crypto.CirculatingSupply, "got", resp.CirculatingSupply)
	}

	if resp.VolumeLastDay != crypto.VolumeLastDay {
		t.Error("expected", crypto.VolumeLastDay, "got", resp.VolumeLastDay)
	}

	if resp.VolumeAllCurrencies != crypto.VolumeAllCurrencies {
		t.Error("expected", crypto.VolumeAllCurrencies, "got", resp.VolumeAllCurrencies)
	}
